<a href="{{ route('detail.landing', $item->id) }}" class="inline-block px-3">
    <div class="w-96 h-auto overflow-hidden md:p-5 p-4 bg-white rounded-2xl inline-block">
        ...
        <!--Banner image-->
        @if (!empty($item->thumbnail_service) && isset($item->thumbnail_service[0]->thumbnail))
            <img class="rounded-2xl object-cover h-48 w-full" src="{{ url(Storage::url($item->thumbnail_service[0]->thumbnail)) }}" alt="service thumbnail" loading="lazy">
        @else   
