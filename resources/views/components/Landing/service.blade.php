<a href="{{ route('detail.landing', $item->id) }}" class="inline-block px-3">
    <div class="w-96 h-auto overflow-hidden md:p-5 p-4 bg-white rounded-2xl inline-block">
        <div class="flex items-center space-x-2 mb-6">
            @if ($item->user && $item->user->detail_user && $item->user->detail_user->photo)
                <img class="w-14 h-14 object-cover rounded-full mr-1" 
                     src="{{ url(Storage::url($item->user->detail_user->photo)) }}" 
                     alt="photo freelancer" loading="lazy">
            @else
                <svg class="inline w-12 h-12 mr-3 text-gray-300 rounded-full" 
            ...
            <div>
                ...
                <p class="text-serv-text font-light text-md">
                    {{ $item->user->detail_user->role ?? 'Unknown Role' }}
                </p>
            </div>
        </div>

        <!-- Banner image -->
        @if (isset($item->thumbnail_service[0]) && $item->thumbnail_service[0]->thumbnail)
...
@endif
        ...
        <div class="text-center mt-5 flex justify-between w-full">
            ...
            <span class="text-serv-button inline-flex items-center leading-none text-md font-semibold">
                Rp {{ number_format($item->price) ?? 'N/A' }}
            </span>
        </div>
    </div>
</a>
